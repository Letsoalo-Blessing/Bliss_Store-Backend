                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <strong class="card-title">Edit product in Stock</strong>
                        </div>
                        <div class="card-body" ng-app="myApp" ng-controller="ProductCtrl">
                            <table class="table table-dark">
                                ...
                                <tbody>
                                    <tr tr ng-repeat="x in products">
                                        <th scope="row">{{x.prodCode}}</th>
                                        <td>{{x.prodID}}</td>
                                        <td>{{x.Description}}</td>
                                        <td>{{x.Category}}</td>
                                        <td>{{x.Qty}}</td>
                                        <td><span>R </span>{{x.UnitPrice}}</td>
                                        <!--data-toggle="modal" data-target="#staticModal"-->
                                        <td><a class="btn btn-primary btn-sm" href="#" data-toggle="modal" data-target="#mediumModal">Edit</a></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <!-- .Update Modal -->
                <div class="modal fade" id="mediumModal" tabindex="-1" role="dialog" aria-labelledby="mediumModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="mediumModalLabel">Update Product</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                            <form action="edit_product.php" method="post" enctype="multipart/form-data" class="form-horizontal">
                                <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Product ID</label></div>
                                    <div class="col-12 col-md-9">
                                        <input type="text" id="txtprodID" name="prodID" placeholder="Enter Product ID" class="form-control">
                                    </div>
                                </div>
                                ...
                                <div class="row form-group">
                                    ...
                                    <div class="col-12 col-md-9">
                                        <input type="text" id="txtDescr" name="Descr" placeholder="Enter Product Description" class="form-control">
                                    </div>
                                </div>
                                <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Quantity</label></div>
                                    <div class="col-12 col-md-9">
                                        <input type="number" id="txtQty" name="Qty" placeholder="Enter Product Quantity" class="form-control">
                                    </div>
                                </div>
                                <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Unit Price</label></div>
                                    <div class="col-12 col-md-9">
                                        <input type="text" id="txtunitPrice" name="unitPrice" placeholder="Enter Product Price" class="form-control">
                                    </div>
                                </div>
                                    <div class="row form-group">
                                        ...
                                        <button type="submit" class="btn btn-primary btn-sm" name="update_product">
                ...
                if(isset($_POST['update_product'])){
                     ...
                     $prod_qty = $_POST['Qty'];
                     $prod_price = $_POST['unitPrice'];
                     $prod_category = $_POST['category'];

                     $update_product = "update tblproduct set Descr='$prod_Descr',Qty='$prod_qty',unitPrice='$prod_price',category='$prod_category' where prodID='$prod_ID'";
    ...
    <script>
        var app = angular.module('myApp', []);
            app.controller('ProductCtrl', function($scope, $http) {
                ...
                // Declarations
                $scope.obj = { 'obj.proIdDisabled': false };
                
                //view products in tables
                $http.get("view_mysql.php")
                    .then(function (response) {$scope.products = response.data.records;});
                
                // Edit DB row item function
        	    $scope.editProduct = function (prodID, descr,qty,unitPrice,category) {
                ...
                $scope.obj = { 'obj.proIdDisabled': true };
                $scope.products();
